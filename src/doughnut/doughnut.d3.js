...
export function dChart() {
  ...
  let colour = d3.scaleOrdinal(d3.schemeCategory10) // colour scheme
  let variable // value in data that will dictate proportions on chart
  let category // compare data by
  let padAngle // effectively dictates the gap between slices
  let transTime // transition time
  let updateData
  let floatFormat = d3.format('.4r')
  let cornerRadius // sets how rounded the corners are on each slice
  // percentFormat = d3.format(',.2%');
  let percentFormat = d3.format('')
  ...
  function chart(selection) {
    selection.each(function () {
      ...
      // construct and arc generator. This will be used for the donut. The difference between outer and inner
      ...
      function labelTween(d) {
        ...
        return function (t) {
          let d2 = interpolate(t)
          let pos = outerArc.centroid(d2) // computes the midpoint [x,y] of the centre line that would be
      ...
      function findFollowing(i, data0, data1, key) {
        let n = data1.length; let m = data0.length
        while (++i < n) {
          let k = key(data1[i])
          for (let j = 0; j < m; ++j) {
            if (key(data0[j]) === k) return data0[j]
